...
export default function PaymentReceipt({ receipt }) {
  ...
  useEffect(() => {
    ...
    const fetchCustomer = async () => {
      // Try identifiers that may come from bookings/receipt
      const ids = [
        receipt.customer_id,
        receipt.Customer_ID,
        receipt.Account_ID,
      ...
      for (const id of ids) {
        ...
        if (acc) {
          // Normalize common account fields to match your SQL `account` table
          setCustomer({
      ...
      const emails = [receipt.customer_email, receipt.client_email, receipt.email].filter(Boolean);
      ...
      // nothing found -> leave customer null (receipt fields used as fallback)
    };
