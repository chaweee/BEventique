...
// Move this function outside of the BookingManagement component
function BookingReceiptWithAccount({ booking }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    // Try all possible account/customer IDs
    const ids = [
      booking.customer_id,
      booking.Customer_ID,
      booking.Account_ID,
      ...
      for (const id of ids) {
        try {
          ...
          if (res.ok) {
            ...
            found = json?.account || json?.data || json?.user || json || null;
            if (found) break;
          }
        } catch {}
      }
  ...
  // Try to get customer info from account or session user
  let customerName = "Account Not Found";
  let customerEmail = "";
  let customerPhone = "";
  
  // First try the fetched account
  if (account) {
    customerName = [
      account.FirstName || account.firstname || "",
      account.M_I || account.mi || "",
      account.LastName || account.lastname || "",
    ]
      .filter(Boolean)
      .join(" ")
      .trim() || account.Full_Name || account.name || "Account Not Found";
    customerEmail = account.Email || account.email || "";
    customerPhone = account.PhoneNumber || account.phone || "";
  }
  
  // If account not found or name is still "Account Not Found", try session user as fallback
  if (customerName === "Account Not Found") {
    try {
      const sessionUser = JSON.parse(sessionStorage.getItem("user") || "{}");
      console.log("Session user data:", sessionUser);
      
      // Build name from firstname and lastname
      const firstName = sessionUser.firstname || sessionUser.FirstName || sessionUser.first_name || "";
      const middleInitial = sessionUser.mi || sessionUser.M_I || sessionUser.middle_initial || "";
      const lastName = sessionUser.lastname || sessionUser.LastName || sessionUser.last_name || "";
      ...
      // Get email and phone
      customerEmail = sessionUser.email || sessionUser.Email || sessionUser.e_mail || "";
      customerPhone = String(sessionUser.phone || sessionUser.PhoneNumber || sessionUser.phone_number || sessionUser.contact || "");
  ...
  // Compose receipt props
  const total = Number(booking.total_amount ?? booking.Package_Amount ?? 0);
  ...
  const receipt = {
    ...
    payment_status: (due <= 0 || booking.status === "confirmed") ? "Paid" : undefined,
  };

  if (loading) return <div style={{ padding: 24 }}>Loading account info...</div>;
